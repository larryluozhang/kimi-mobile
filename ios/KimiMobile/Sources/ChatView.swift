...
struct ChatView: View {
    ...
    var body: some View {
        ...
        // / 命令结果接线：/fork 切新会话、/archive 返回列表、/new 返回列表并新建、/help 弹窗
        ...
        .alert("可用命令", isPresented: $vm.showSlashHelp) {
            ...
            Text(ChatViewModel.slashHelpText)
        }
        .onChange(of: vm.requestExitToList) { exit in
            if exit { dismiss() }
        }
        .onChange(of: vm.requestNewSession) { new in
            if new {
