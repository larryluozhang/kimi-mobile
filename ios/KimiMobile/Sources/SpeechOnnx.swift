...
final class SpeechOnnx: ObservableObject {
    ...
    /// 模型在 bundle 中的目录（project.yml 以 folder reference 原样拷贝 models/）
    static let modelSubdir = "models/zipformer-bilingual"
    ...
    private static let tokensFile = "tokens.txt"

    /// 模型文件是否已打进 bundle（四个文件齐全才算可用）
    ...
    private static var modelDirURL: URL? {
        ...
        let dir = base.appendingPathComponent(modelSubdir)
    ...
    /// 从 bundle 构建识别器；模型缺失或初始化失败返回 nil
